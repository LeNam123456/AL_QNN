...
def load_raw_dataset(dataset_cfg: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, list[str], list[str]]:
    ...
    if name == DatasetName.IRIS.value:
        ...
        data = load_digits()
    elif name == DatasetName.PIMA_DIABETES.value:
    ...
    elif name == DatasetName.HEART_DISEASE.value:
        from sklearn.datasets import fetch_openml
        ...
        return X, y, feature_names, class_names
    elif name == DatasetName.PARKINSONS.value:
        from sklearn.datasets import fetch_openml
        ...
        raise ValueError(
            ...
            "Supported: iris, wine, breast_cancer, digits, pima_diabetes, heart_disease, parkinsons."
        )
...
def select_features(
    ...
    if method == "pca":
        ...
        X_train_selected = pca.fit_transform(X_train_raw)
        X_val_selected = pca.transform(X_val_raw)
        X_test_selected = pca.transform(X_test_raw)
        
        report = FeatureSelectionReport(
            method=method,
            selected_indices=list(range(n_features)),
            selected_feature_names=[f"pca_component_{i}" for i in range(n_features)],
            variance_threshold=None,
            score_func=None,
        )
        return X_train_selected, X_val_selected, X_test_selected, report
    ...
    raise ValueError(
        ...
        "Use first_n, variance_threshold, variance_then_selectkbest, or pca."
    )
